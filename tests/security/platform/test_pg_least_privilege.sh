...
echo "TEST-SEC-001/015 pg least privilege"
PG="${PREFIX}-pg"
...
soviez_sec_pg_provision_least_privilege "$PG" soviez_admin "$ADMIN_PASS" soviez_app "$APP_PASS" "s1db_${RUN_ID}"
soviez_sec_pg_assert_app_role_safe "$PG" soviez_admin "$ADMIN_PASS" soviez_app postgres
attrs="$(soviez_sec_pg_query_role_attrs "$PG" soviez_admin "$ADMIN_PASS" soviez_app postgres)"
# PG may emit f or false depending on version/settings
case "$attrs" in
  f,f,f,f,f|false,false,false,false,false) ;;
  *) echo "FAIL attrs=$attrs"; exit 1 ;;
esac
echo "PASS TEST-SEC-001/015"
