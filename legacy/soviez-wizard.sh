...
# RETIRED: unsigned public self-update is no longer available.
# Use the canonical modular platform + signed self-update via /usr/local/bin/soviez.sh.
update_self() {
  echo "[error] legacy unsigned updater (update_self) is retired and inaccessible" >&2
  echo "[error] install/use signed Soviez.sh platform: curl -sSL https://soviez.sh | sudo bash" >&2
  echo "[error] then: soviez.sh --version" >&2
  return 1
}

# Do not auto-invoke update_self. Legacy wizard is migration-only (not public runtime).
# update_self "$@"  # RETIRED — unsigned body replacement forbidden
